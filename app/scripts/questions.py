...
logging.basicConfig(filename = 'log.txt', filemode = 'a+', format = '%(asctime)s %(name)s: %(levelname)s: %(message)s', datefmt = '%Y-%m-%d %H:%M:%S', level = logging.INFO)
...
id += 1
query_dict.update({str(id): '以下哪些是你比较感兴趣的（可多选）'})
opt_dict.update({str(id): {'a': '传媒', 'b': '工业设计', 'c': '文史哲学', 'd': '表演/影视剧作', 'e': '跳过'}})
total_dict.update({str(id): question(id, query_dict[str(id)], opt_dict[str(id)])})
...
opt_dict.update({str(id): {'a': '流行', 'b': '古典', 'c': '嘻哈', 'd': '小众其它', 'e': '跳过'}})
total_dict.update({str(id): question(id, query_dict[str(id)], opt_dict[str(id)])})
...
id += 1
query_dict.update({str(id): '你可能感兴趣（可多选）'})
opt_dict.update({str(id): {'a': '中国古代文学历史', 'b': '西方近现代哲学', 'c': '当代文学', 'd': '其它文史哲学', 'e': '跳过'}})
total_dict.update({str(id): question(id, query_dict[str(id)], opt_dict[str(id)])})
...
id += 1
query_dict.update({str(id): '你可能感兴趣（可多选）'})
opt_dict.update({str(id): {'a': '狼人杀', 'b': '阿瓦隆', 'c': '卡坦岛', 'd': '其它', 'e': '跳过'}})
total_dict.update({str(id): question(id, query_dict[str(id)], opt_dict[str(id)])})
...
id += 1
query_dict.update({str(id): '（可多选）'})
opt_dict.update({str(id): {'a': '', 'b': '', 'c': '', 'd': '', 'e': '跳过'}})
total_dict.update({str(id): question(id, query_dict[str(id)], opt_dict[str(id)])})
...
ansdict = dict()
'''
def launch(i: int, launch_id: int):
    launch_id += 1
    try:
        q = total_dict[str(i)]
        answer = ''
        ans = set()
        while not ans <= set(q.options.keys()) or ans == set():
            # answer = input('\n' + str(launch_id) + '. ' + q.query + '\n' + q.list_options() + '\n')
            answer = input('\n' + q.query + '\n' + q.list_options() + '\n')
            ans.update(answer.split(' '))
        logging.info('\n' + str(q.id) + '\n' + ' '.join([q.options[key] for key in answer.split(' ')]) + '\n')
        ansdict.update({str(i): answer.split(' ')})
    except KeyError:
        print('\n个人信息收集完成。\n')
    return launch_id

def colaunch(mixlist: list, launch_id: int):
    for i in range(len(mixlist)):
        launch_id = launch(mixlist[i], launch_id)
    return launch_id

def LaunchLayer(id: int, launch_id: int, tmplist = []):
    mixlist = mix(tmplist)

    if id == 1:
        while mixlist.index(6) > mixlist.index(7):
            mixlist = mix(tmplist)
        launch_id = colaunch(mixlist, launch_id)
    if id == 2:
        launch_id = colaunch(mixlist, launch_id)
    if id == 3:
        launch_id = colaunch(mixlist, launch_id)
...
def LaunchTotal(id: int, ansdict = dict()):
    ...
    if id == 1:
        ...
        # layerdict.update({1: dict()})
        # layer, launch_id = LaunchLayer(1, launch_id, tmplist)
        # layer = LaunchLayer(1, launch_id, tmplist)
        # for item in layer:
            # layerdict[1].update({item: ansdict[str(item)]})
    ...
    if id == 2:
        ...
        if ansdict != dict():
            ...
            if 'b' in ansdict['5'] or 'c' in ansdict['5']:
                tmplist.append(27)
            tmplist.append(28)
            # layerdict.update({2: dict()})
            # layer, launch_id = LaunchLayer(2, launch_id, tmplist)
            # layer = LaunchLayer(2, launch_id, tmplist)
            # for item in layer:
                # layerdict[2].update({item: ansdict[str(item)]})
                # fill(item, query_dict, opt_dict, dict_dict)
            ttlist = mix(tmplist)
            for item in ttlist:
                fill(item, query_dict, opt_dict, dict_dict)
    ...
    if id == 3:
        ...
        if ansdict != dict():
            ...
            if '17' in ansdict.keys():
                ...
                if 'c' in ansdict['17']:
                    tmplist.append(32)
            # layerdict.update({3: dict()})
            # layer, launch_id = LaunchLayer(3, launch_id, tmplist)
            # layer = LaunchLayer(3, launch_id, tmplist)
            # for item in layer:
                # layerdict[3].update({item: ansdict[str(item)]})
                # fill(item, query_dict, opt_dict, dict_dict)
            ttlist = mix(tmplist)
            for item in ttlist:
                fill(item, query_dict, opt_dict, dict_dict)
    ...
    tmplist = []
    # layer, launch_id = LaunchLayer(99, launch_id)
